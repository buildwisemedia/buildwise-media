...
- **Fit CTA** (`qa:brand` `locked-cta-present`; website QA mobile-hero CTA). Bob pages now need the approved header action "See Bob at Work", visible inside the site header. Legacy routes still need the fit CTA. The route list and a real `rel="stylesheet"` link to `/bob/site.css` are checked against each other (`bob-surface-registry`), so a page cannot move between rule sets silently.
- **Sample demos**: only the two approved demos (`/bob/proof/hope-demo-r4`, `/bob/proof/service-demo-r43`) skip the contact CTA. They need `noindex` and a title or description that calls them a sample. The page that embeds each one must label it a sample beside the frame. Any other `/bob/proof/` page fails. They still need one H1 and a meta description.
- **Typefaces** (Pre-Ship `banned-typefaces`). Manrope and DM Sans may appear only as `@font-face` declarations in `public/bob/site.css` and the six `src/data/bob/*.html` pages, read from Brain `brand/BWM-Brand-Tokens.json`. The site CSS must map `--head` to Manrope and `--body` to DM Sans. Everywhere else they stay banned, and every other face stays banned everywhere.
