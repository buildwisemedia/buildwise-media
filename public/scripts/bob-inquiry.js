...
export function cleanSource(raw, base) {
  ...
  try {
    ...
    for (const key of CAMPAIGN_KEYS) {
      ...
      if (value && !/[@\r\n]/.test(value)) clean.searchParams.set(key, value.slice(0, 160));
    }
...
export function initInquiry(form) {
  ...
  let failed = null;
  let sending = false;
  if (!['buildwisemedia.com','www.buildwisemedia.com'].includes(location.hostname)) {
    status.textContent='This is a test copy. Please use buildwisemedia.com to send your request.';status.hidden=false;return;
  }
  button.disabled = false;
  const track = (name, submissionId, selectedInterest) => {
    if (!['buildwisemedia.com', 'www.buildwisemedia.com'].includes(location.hostname)) return;
    if (typeof window.__bwmTrackEvent === 'function') window.__bwmTrackEvent(name, {
      submission_id: submissionId, form_id: `bob_${kind}`, request_kind: kind, ...(kind === 'fit' ? {selected_interest: selectedInterest ?? ''} : {}),
    });
  };
  form.addEventListener('submit', async (event) => {
    ...
    const content = {
      request_kind: kind,
      ...(kind === 'fit' ? {selected_interest: fields.selected_interest?.value || ''} : {}),
    ...
    const payload = failed?.fingerprint === fingerprint ? failed.payload : {
      ...
      form_started_at: startedAt, ...content, attribution: typeof window.__bobSourceDetails === 'function' ? window.__bobSourceDetails() : sourceDetails(location.href, document.referrer),
    };
    sending = true;
    ...
    try {
      ...
      failed = null;
      track(kind === 'fit' ? 'fit_note_submitted' : `${kind}_request_submitted`, payload.submission_id, payload.selected_interest);
      track('generate_lead', payload.submission_id, payload.selected_interest);
      form.hidden = true;
      ...
      status.hidden = false;
      status.focus();
      track('form_submit_error', payload.submission_id, payload.selected_interest);
    } finally {
